""" Evaluate available methods on the Gaussian dataset.

The results are saved to results/gaussian_results.pkl.
"""
import os
import joblib
from independence_test.utils import pc_ks
from independence_test.data import make_gaussian_data as make_data
...
SAVE_FNAME = 'gaussian_results.pkl'
...
if __name__ == "__main__":
    ...
    for n_samples in SAMPLE_NUMS:
        for dim in [1, 10, 100, 1000]:
            for zdim in [1, 10, 100, 1000]:
                xd, yd, zd = make_data(
                    type='dep', n_samples=n_samples,
                    xdim=dim, ydim=dim, zdim=zdim)

                xi, yi, zi = make_data(
                    type='indep', n_samples=n_samples,
                    xdim=dim, ydim=dim, zdim=zdim)

                for method_name in METHODS:
                    # Set up the storage.
                    key = '{}_{}mt_{}samples_{}dim_{}complexity'.format(
                        method_name, MAX_TIME, n_samples, dim, zdim)
