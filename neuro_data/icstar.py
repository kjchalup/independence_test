...
I based this implementation on work of Adam Kelleher
(see his github page at https://github.com/akelleh).
"""
...
class IC():
    def __init__(self, alpha=0.05, verbose=True, testargs={}):
        self.testargs = testargs
        self.independence_test = fit.test
        self.alpha = alpha
        ...
        for c in self._g.nodes():
            for (a,b) in itertools.combinations(self._g.neighbors(c), 2):
                if not self._g.has_edge(a,b):
                    if (c in self._g[a][c]['arrows'] and
                        c not in self._g[b][c]['arrows'] and
                        not (b in self._g[b][c]['arrows'] and
                            self._g[b][c]['marked'])):
                        self._g[b][c]['arrows'].append(b)
                        self._g[b][c]['marked'] = True
                        added_arrows = True
                    if (c in self._g[b][c]['arrows'] and
                        c not in self._g[a][c]['arrows'] and
                        not (a in self._g[a][c]['arrows'] and
                            self._g[a][c]['marked'])):
    ...
    def _marked_directed_path(self,a,b):
        ...
        while neighbors:
            ...
            if (child in self._g[parent][child]['arrows'] and
                self._g[parent][child]['marked']):
                if child == b:
                    return True
                if child not in seen:
                    neighbors += [(child, neighbor)
                        for neighbor in self._g.neighbors(child)]
                seen.append(child)
        return False
        ...
        for N in range(len(self._g.node)+1):    
            for (x, y) in self._g.edges():
                ...
                for z in itertools.combinations(z_candidates, N):
                    # Extract and concatenate the variables.
                    x_data = data[x]
                    y_data = data[y]
                    if len(z) > 0:
                        z_data = np.hstack([data[zvar] for zvar in z])
                    else:
                        z_data = None

                    # Print out info and run the test.
                    if self.verbose:
                        logging.info("=" * 70)
                        logging.info("Testing if {} indep {} given {}".format(x, y,
                            z if len(z) > 0 else 'None'))
                    t = time.time()
                    pval = self.independence_test(
                        x_data, y_data, z_data, **self.testargs)
                    if self.verbose:
                        logging.info("Done. pval = {}, test time = {}s".format(
                            pval, time.time() - t))
                        logging.info("=" * 70)
